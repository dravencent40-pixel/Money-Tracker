...
@section('content')
    ...
    <form method="POST" action="{{ route('categories.store') }}" class="bg-white border border-slate-200 rounded-lg border border-slate-200 p-4 max-w-sm space-y-3">
        @csrf
        <input type="text" name="name" placeholder="Nama kategori" required class="w-full rounded-md border-slate-300 bg-slate-100 text-slate-700 text-sm">
